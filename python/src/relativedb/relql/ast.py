...
class MissingParameterError(KeyError):
    """A ``:name`` in the query had no value supplied at execution time."""

    def __init__(self, name: str, supplied) -> None:
        self.name = name
        have = ", ".join(sorted(map(str, supplied))) or "none"
        super().__init__(
...
class AggFunc(Enum):
    ...
    LIST_DISTINCT = "LIST_DISTINCT"   # set of distinct values
    ARRAY_AGG = "ARRAY_AGG"           # values in order, duplicates kept
    FIRST = "FIRST"
    LAST = "LAST"
...
class Window:
    ...
    An aggregation with no ``OVER`` clause gets an implied unbounded frame,
    pointing in the direction of the clause it sits in: the future
    ``(0, +inf]`` in PREDICT/ASSUMING, the past ``(-inf, 0]`` in WHERE.
    ``implied`` records that, so EXPLAIN can tell a default frame apart from an
    explicit ``UNBOUNDED`` one.
    """
    ...
    top_k: Optional[int] = None             # RANK TOP k directive on this frame
    implied: bool = False                   # frame defaulted, not written
...
@dataclass(frozen=True)
class Param:
    """A ``:name`` bind parameter standing in for a literal, or — for
    ``IN :name`` — for the whole list. :meth:`ParsedQuery.bind_params`
    replaces these with values before evaluation."""

    name: str


TargetExpr = Union[Aggregation, ColumnRef, Condition, LogicalOp, Not,
                   Arith, Func, Case, Lit, Param]


def _bind(expr, params: dict) -> Any:
    """Rewrite ``expr``, replacing every :class:`Param` with its bound value.

    A parameter in comparison-RHS position collapses onto ``Condition.right``
    (the literal slot) so downstream evaluation never has to know parameters
    existed. Lists bind to tuples, matching how ``IN`` literals parse.
    """
    if expr is None:
        return None
    if isinstance(expr, Param):
        if expr.name not in params:
            raise MissingParameterError(expr.name, params.keys())
        return Lit(_bind_value(params[expr.name]))
    if isinstance(expr, Condition):
        left = _bind(expr.left, params)
        if isinstance(expr.right_expr, Param):
            if expr.right_expr.name not in params:
                raise MissingParameterError(expr.right_expr.name, params.keys())
            return Condition(left, expr.op,
                             _bind_value(params[expr.right_expr.name]), None)
        return Condition(left, expr.op, expr.right,
                         _bind(expr.right_expr, params))
    if isinstance(expr, LogicalOp):
        return LogicalOp(_bind(expr.left, params), expr.op,
                         _bind(expr.right, params))
    if isinstance(expr, Not):
        return Not(_bind(expr.expr, params))
    if isinstance(expr, Arith):
        return Arith(expr.op, _bind(expr.left, params),
                     _bind(expr.right, params))
    if isinstance(expr, Func):
        return Func(expr.name, tuple(_bind(a, params) for a in expr.args))
    if isinstance(expr, Case):
        whens = tuple((_bind(c, params), _bind(t, params))
                      for c, t in expr.whens)
        return Case(whens, _bind(expr.else_, params))
    if isinstance(expr, Aggregation):
        return Aggregation(expr.func, expr.column, _bind(expr.filter, params),
                           expr.window)
    return expr


def _bind_value(v: Any) -> Any:
    """Normalize a bound value: sequences become tuples (the shape ``IN``
    literals already have); scalars pass through."""
    if isinstance(v, (list, tuple, set, frozenset)):
        return tuple(v)
    return v


def _has_params(expr) -> bool:
    if expr is None:
        return False
    if isinstance(expr, Param):
        return True
    if isinstance(expr, Condition):
        return _has_params(expr.left) or _has_params(expr.right_expr)
    if isinstance(expr, LogicalOp):
        return _has_params(expr.left) or _has_params(expr.right)
    if isinstance(expr, Not):
        return _has_params(expr.expr)
    if isinstance(expr, Arith):
        return _has_params(expr.left) or _has_params(expr.right)
    if isinstance(expr, Func):
        return any(_has_params(a) for a in expr.args)
    if isinstance(expr, Case):
        return (any(_has_params(c) or _has_params(t) for c, t in expr.whens)
                or _has_params(expr.else_))
    if isinstance(expr, Aggregation):
        return _has_params(expr.filter)
    return False
...
class ParsedQuery:
    ...
    target: TargetExpr
    # The population. ``entity_key.table`` comes from ``FROM <table>``, or is
    # inferred from the target's own table when the query has no FROM clause
    # (``entity_inferred``). ``entity_key.column`` is the primary key, which the
    # parser cannot know without a schema: it is None until :func:`validate`
    # binds it via :meth:`bind_entity_key`.
    entity_key: ColumnRef
    entity_inferred: bool = False
    ...
    def bind_params(self, params: Optional[dict]) -> "ParsedQuery":
        ...
        return dataclasses.replace(
            self,
            target=_bind(self.target, p),
            where=_bind(self.where, p),
            assuming=_bind(self.assuming, p))

    def bind_entity_key(self, schema) -> "ParsedQuery":
        """Return this query with ``entity_key.column`` filled in from the
        schema's declared primary key. A no-op once bound."""
        import dataclasses
        if self.entity_key.column:
            return self
        table = schema.table(self.entity_key.table)
        if table is None or not table.primary_key:
            return self
        return dataclasses.replace(
            self, entity_key=ColumnRef(self.entity_key.table,
                                       table.primary_key))
    ...
    def task_type(self, schema=None) -> TaskType:
        ...
        if isinstance(t, Aggregation):
            if t.func is AggFunc.EXISTS:
                return TaskType.BINARY_CLASSIFICATION
            if t.func in (AggFunc.LIST_DISTINCT, AggFunc.ARRAY_AGG):
                return TaskType.MULTILABEL_RANKING
