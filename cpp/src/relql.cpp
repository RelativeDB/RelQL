...
const std::unordered_set<std::string>& softKeywords() {
  static const std::unordered_set<std::string> s = [] {
    ...
    for (const char* w : {"PREDICT", "FROM", "WHERE", "ASSUMING", "AND", "OR",
                          "NOT", "NULL"})
      out.erase(w);
    return out;
  }();
...
[[noreturn]] void syntaxError(const std::string& message, size_t pos,
  ...
  throw RelqlError("RelQL syntax error" + loc + ": " + message + snippet);
}
...
AggFunc aggFuncFromName(const std::string& k) {
  static const std::unordered_map<std::string, AggFunc> m = {
      ...
      {"LIST_DISTINCT", AggFunc::LIST_DISTINCT},
      {"ARRAY_AGG", AggFunc::ARRAY_AGG}, {"FIRST", AggFunc::FIRST},
...
class Parser {
 public:
  ...
  ParsedQuery parseQuery() {
    ...
    q.target = parseExpr();
    if (accept("CLASSIFY")) q.rank = RankKind::CLASSIFY;
    parseFrom(q);
    parseTrailingClauses(q);
    expect("EOF", "end of query");
    resolveWindowRefs(q);
    resolveNames(q);
    applyImpliedWindows(q);
    deriveForecasts(q);
    deriveRank(q);
    return q;
  }
 ...
 private:
  ...
  std::string from_alias_;               // FROM <table> <alias>, if given
  ...
  void parseFrom(ParsedQuery& q) {
    if (!accept("FROM")) return;
    q.has_from = true;
    q.entity_table = parseName("a table name after FROM");
    // `AS` also opens `AS OF`; only treat it as an alias marker otherwise.
    if (peek().kind == "AS" && peek(1).kind != "OF") next();
    const Token& t = peek();
    if ((t.kind == "IDENT" || softKeywords().count(t.kind)) &&
        !clauseStarters().count(t.kind))
      from_alias_ = next().text;
  }
  ...
  static const std::unordered_set<std::string>& clauseStarters() {
    static const std::unordered_set<std::string> s = {
        "AS", "ABLATE", "RETURN", "WINDOW", "WHERE", "ASSUMING"};
    return s;
  }

  // Infer the population from the target when there is no FROM clause.
  void inferEntityTable(ParsedQuery& q) {
    std::vector<const Expr*> aggs;
    collectAggs(q.target.get(), aggs);
    if (!aggs.empty())
      syntaxError(
          "an aggregate target needs a FROM clause to name the population "
          "(the aggregation's table is a linked table, not the population)",
          0, text_);
    std::vector<std::string> tables;
    collectTables(q.target.get(), tables);
    if (tables.empty())
      syntaxError("cannot infer the population: PREDICT names no table; "
                  "add a FROM clause",
                  0, text_);
    for (const std::string& t : tables)
      if (t != tables.front())
        syntaxError("cannot infer the population: PREDICT spans tables '" +
                        tables.front() + "' and '" + t +
                        "'; add a FROM clause",
                    0, text_);
    q.entity_table = tables.front();
  }
  ...
  void resolveNames(ParsedQuery& q) {
    if (!q.has_from) inferEntityTable(q);
    std::vector<Expr*> cols;
    collectColumns(q.target.get(), cols);
    collectColumns(q.where.get(), cols);
    collectColumns(q.assuming.get(), cols);
    for (Expr* e : cols) {
      if (e->table.empty())
        e->table = q.entity_table;      // unqualified -> the population
      else if (!from_alias_.empty() && e->table == from_alias_)
  ...
  // in EXPLAIN.
  void applyImpliedWindows(ParsedQuery& q) {
    impliedInto(q.target.get(), /*future=*/true);
    impliedInto(q.assuming.get(), /*future=*/true);
    impliedInto(q.where.get(), /*future=*/false);
  }

  void impliedInto(Expr* root, bool future) {
    std::vector<const Expr*> aggs;
    collectAggs(root, aggs);
    for (const Expr* c : aggs) {
      Expr* a = const_cast<Expr*>(c);
      if (a->has_window) continue;
      Window w;
      w.unit = TimeUnit::DAYS;
      w.implied = true;
      w.start = future ? 0.0 : -std::numeric_limits<double>::infinity();
      w.end = future ? std::numeric_limits<double>::infinity() : 0.0;
      a->window = w;
      a->has_window = true;
    }
  }
  ...
  ExprPtr parseParam() {
    expect(":", "':' to open a bind parameter");
    auto e = std::make_shared<Expr>();
    e->kind = ExprKind::Param;
    e->param_name = parseName("a bind parameter name after ':'");
    return e;
  }

  // The RHS of a word operator (LIKE, CONTAINS, STARTS WITH, ...): a literal,
  // or a `:name` parameter standing in for one.
  void parseRhsLiteralOrParam(const ExprPtr& cond) {
    if (peek().kind == ":") {
      cond->right_expr = parseParam();
    } else {
      cond->right = parseLiteral();
    }
    cond->has_right = true;
  }
  ...
  void parseRhsListOrParam(const ExprPtr& cond) {
    if (peek().kind == ":") {
      cond->right_expr = parseParam();
    } else {
      cond->right = listLit(parseListLiteral());
    }
    cond->has_right = true;
  }
  ...
  Window parseWindowSpec() {
    ...
    if (peek().kind == "RANK") {
      ...
      w.implied = true;
      w.start = 0.0;
      w.end = std::numeric_limits<double>::infinity();
      parseRankTop(w);
      return w;
    }
    if (accept("RANGE")) {
  ...
  void parseRankTop(Window& w) {
    if (peek().kind != "RANK") return;
    next();
    expect("TOP", "'TOP' after RANK");
    Token k = expect("INT", "an integer after RANK TOP");
    if (k.ival < 1)
      syntaxError("RANK TOP must be a positive integer", k.pos, text_);
    w.has_top_k = true;
    w.top_k = k.ival;
  }
  ...
  // `table.column`, `alias.column`, `table.*`, or a bare `column`. A bare name
  // leaves `table` empty for `resolveNames` to bind to the population.
  void parseColumnRefInto(std::string& table, std::string& column) {
    std::string name = parseName("a column or table name");
    if (!accept(".")) {
      column = name;
      return;
    }
  ...
  static void collectColumns(Expr* e, std::vector<Expr*>& out) {
    if (!e) return;
    switch (e->kind) {
      case ExprKind::Col: out.push_back(e); break;
      case ExprKind::Agg:
        out.push_back(e);
        collectColumns(e->filter.get(), out);
        break;
      case ExprKind::Cond:
        collectColumns(e->left.get(), out);
        collectColumns(e->right_expr.get(), out);
        break;
      case ExprKind::Logic:
        collectColumns(e->rleft.get(), out);
        collectColumns(e->rright.get(), out);
        break;
      case ExprKind::Not: collectColumns(e->inner.get(), out); break;
      case ExprKind::Arith:
        collectColumns(e->a_left.get(), out);
        collectColumns(e->a_right.get(), out);
        break;
      case ExprKind::Func:
        for (auto& a : e->args) collectColumns(a.get(), out);
        break;
      case ExprKind::Case:
        for (auto& c : e->when_conds) collectColumns(c.get(), out);
        for (auto& th : e->when_thens) collectColumns(th.get(), out);
        collectColumns(e->case_else.get(), out);
        break;
      default: break;
    }
  }
  ...
  static void collectTables(Expr* e, std::vector<std::string>& out) {
    std::vector<Expr*> cols;
    collectColumns(e, cols);
    for (const Expr* c : cols) {
      if (c->table.empty()) continue;
      bool seen = false;
      for (const std::string& t : out) seen = seen || t == c->table;
      if (!seen) out.push_back(c->table);
    }
  }
  ...
  void deriveRank(ParsedQuery& q) {
    std::vector<const Expr*> aggs;
    collectAggs(q.target.get(), aggs);
    for (const Expr* a : aggs) {
      if (!a->has_window || !a->window.has_top_k) continue;
      if (q.has_top_k && q.top_k != a->window.top_k)
        syntaxError("conflicting RANK TOP values in one target", 0, text_);
...
void emitWindow(std::string& out, const Window& w) {
  ...
  out += ",\"top_k\":";
  if (w.has_top_k)
    out += std::to_string(w.top_k);
  else
    out += "null";
  out += ",\"implied\":";
  out += w.implied ? "true" : "false";
  out += "}";
}
...
void emitExpr(std::string& out, const Expr& e) {
  switch (e.kind) {
    ...
    case ExprKind::Param:
      out += "{\"kind\":\"param\",\"name\":";
      emitJsonString(out, e.param_name);
      out += "}";
      break;
    case ExprKind::Cond:
      out += "{\"kind\":\"cond\",\"left\":";
...
ParsedQuery parse(const std::string& query) {
  ...
  if (blank) throw RelqlError("RelQL syntax error: empty query");
  Parser p(query);
  return p.parseQuery();
}
...
std::string to_json(const ParsedQuery& q) {
  ...
  out += ",\"column\":";
  if (q.entity_column.empty())
    out += "null";  // primary key unresolved; the binding fills it from schema
  else
    emitJsonString(out, q.entity_column);
  out += ",\"inferred\":";
  out += q.has_from ? "false" : "true";
  out += "},\"where\":";
  if (q.where)
