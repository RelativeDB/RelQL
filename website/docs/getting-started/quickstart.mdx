...
and fetching the checkpoint. The current single-head checkpoint supports binary
classification and regression; multiclass, ranking, and `RETURN
QUANTILES`/`INTERVAL` are not supported.
:::
